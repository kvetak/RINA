...
template<class T>
class TSimpleLS: public IntTSimpleRouting<T> {
protected:
    string myAddr;
...
public:
    ...
    bool processUpdate(IntRoutingUpdate * update) {
        if(Update * u = dynamic_cast<Update*>(update)) {
            if(modules.find(u->qos) == modules.end()) {
                modules[u->qos] = LS_Module<T>(this, IntTSimpleRouting<T>::myAddress, myAddr, IntTSimpleRouting<T>::infinite );
            }
    ...
    virtual void insertFlow(const Address &addr, const string &dst, const string& qos, const T &metric) {
        if(modules.find(qos) == modules.end()) {
            modules[qos] = LS_Module<T>(this, IntTSimpleRouting<T>::myAddress, myAddr, IntTSimpleRouting<T>::infinite );
        }
    ...
    virtual void handleMessage(cMessage *msg){
        if(AutoMsg * m = dynamic_cast<AutoMsg*>(msg)) {
            if(modules.find(m->qos) == modules.end()) {
                modules[m->qos] = LS_Module<T>(this, IntTSimpleRouting<T>::myAddress, myAddr, IntTSimpleRouting<T>::infinite );
            }
